...
本文是实现、测试和审查共同使用的现行架构规范，只保留已确认的设计与可验证契约。原始讨论与过程记录由开发者在本地保留。实现与测试引用本文中的契约编号；验收结果和复现命令见 [`zork-agent-status.md`](./zork-agent-status.md)。
...
```json
{ "tool": "aa.bb", "goal": "当前目标", "action": "这次调用的行为", "arguments": {} }
```

`tool` 是逻辑工具完整名称，`arguments` 始终是 JSON object。`goal` 和 `action` 是模型填写的外层展示参数，分别表达当前目标和此次调用行为，不传给具体工具；字段上限和兼容规则见 [tool-activity.md](tool-activity.md)。tool version、provider call ID 和内部 ToolInvocation ULID 都由 runtime 添加，不由 agent填写。固定 schema 不枚举动态工具名。
...
`tool.cancel` 先持久化取消请求，再通知 ToolExecutor，并等到目标真实 ToolResult 持久化后才完成。返回 target_outcome 和 target_result，明确区分取消、提前完成、失败和未知副作用；没有待完成目标时返回 signalled=false，不宣称取消成功。runner 通过完成事件唤醒取消请求方，等待期间继续消费工具结果，不在控制处理函数中阻塞。tool.cancel 同样经过 registry 版本/可用性检查和受监督执行，通过有界控制通道请求 runner 持久化取消事实后发信号，不由 runner 绕过工具入口执行。取消通过有界控制请求通道派发，不占用普通执行名额，也不在等待清理时占用独占取消名额；普通工具满载或另一项取消尚未结束时，仍能取消其他目标；版本检查、解析、超时和结果持久化规则一致。

## 9. Turn、Auto Wait 与重试

### 9.1 Turn（`TURN-01`、`TURN-02`）

模型回复没有工具调用且没有 outstanding 时，正文持久化并展示，turn 自然结束；空回复同样结束。有工具调用时继续执行工具并将结果交给模型。上下文压缩和交接保持各自的完成规则。
