...
The device connection UI generates a pinned command ending in
`-- mesh join '<invitation>'`. Compatible executables on PATH are reused.
Otherwise a verified temporary bundle runs the native command, which discovers
existing nodes and installs missing components into the selected node's `bin`
directory. The bootstrap deletes its temporary files after success or failure.
Services never depend on those files. Repeating installation or joining does
not overwrite an existing installation or restart its running tasks.
...
phones scan or paste the corresponding client invitation. After joining, configure
...
components in an archive come from one source checkout; the embedded Synch crates
use the Git revision pinned in `crates/zork-mesh/Cargo.toml` and `Cargo.lock`.
