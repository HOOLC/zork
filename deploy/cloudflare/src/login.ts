...
export function devicePage(title: string, content: string, cookies?: string): Response {
  return new Response(
    ...
    {
      headers: { "content-type": "text/html; charset=utf-8", "cache-control": "no-store", "referrer-policy": "no-referrer", "content-security-policy": "default-src 'none'; style-src 'unsafe-inline'; form-action 'self'; frame-ancestors 'none'", ...(cookies ? { "set-cookie": cookies } : {}) },
    },
  );
}
