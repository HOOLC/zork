...
impl WebWindowInner {
    ...
    fn create_raf_closure(self: &Rc<Self>) -> Closure<dyn FnMut()> {
        ...
        let closure = Closure::new(move || {
            ...
            if this.gpu_frames_pending.get() >= 2 {
    ...
    pub(crate) fn wake_frame_loop(&self) {
        ...
        // Keep a bounded pipeline. A GPU completion wakes deferred demand;
        // idle windows do not poll or schedule another animation frame.
        if self.gpu_frames_pending.get() >= 2 || self.raf_id.get().is_some() {
            return;
        }
...
impl PlatformWindow for WebWindow {
    ...
    fn draw(&self, scene: &Scene) {
        ...
        let submitted = self.inner.state.borrow_mut().renderer.draw(scene);
        if submitted {
            let (sender, receiver) = futures::channel::oneshot::channel();
            if self.inner.state.borrow().renderer.on_frame_completed(move || { let _ = sender.send(()); }) {
                self.inner.gpu_frames_pending.set(self.inner.gpu_frames_pending.get() + 1);
                let inner = Rc::downgrade(&self.inner);
                wasm_bindgen_futures::spawn_local(async move {
                    let _ = receiver.await;
                    if let Some(inner) = inner.upgrade() {
                        inner.gpu_frames_pending.set(inner.gpu_frames_pending.get().saturating_sub(1));
                        if inner.frame_requested.get() { inner.wake_frame_loop(); }
                    }
                });
            }
        }
