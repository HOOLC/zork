...
impl RelayActor {
    ...
    async fn handle_msg(&mut self, msg: RelayActorMessage) {
        match msg {
            RelayActorMessage::ConfigChanged => self.on_config_change(),
    ...
    fn start_active_relay(&mut self, url: RelayUrl) -> ActiveRelayHandle {
        ...
        let config = self.config.relay_map.get(&url);
        let auth_token = config.as_ref().and_then(|cfg| cfg.auth_token.clone());
...
struct ActiveRelayHandle {
    config: Option<Arc<relay::RelayConfig>>,
    stop_token: CancellationToken,
...
impl std::fmt::Debug for ActiveRelayHandle {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ActiveRelayHandle").finish_non_exhaustive()
    }
}
