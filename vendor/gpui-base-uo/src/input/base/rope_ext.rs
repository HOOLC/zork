...
/// Navigate extended graphemes without flattening the rope. Clusters can span
/// chunks (combining marks, regional indicators and ZWJ emoji).
pub(crate) fn grapheme_boundary(text: &Rope, offset: usize, forward: bool) -> usize {
    use unicode_segmentation::{GraphemeCursor, GraphemeIncomplete};
    let offset = text.clip_offset(offset, Bias::Left);
    if (forward && offset == text.len()) || (!forward && offset == 0) {
        return offset;
    }
    ...
    loop {
        let boundary = if forward {
            cursor.next_boundary(chunk.chunk(), chunk.byte_offset())
        } else {
            cursor.prev_boundary(chunk.chunk(), chunk.byte_offset())
        };
        match boundary {
            ...
            Err(GraphemeIncomplete::PreContext(end)) => {
                let context = text.chunk_cursor_at(end - 1);
                cursor.provide_context(
                    &context.chunk()[..end - context.byte_offset()],
                    context.byte_offset(),
                );
            }
            Err(GraphemeIncomplete::InvalidOffset) => {
                unreachable!("rope chunk contains the grapheme cursor")
            }
        }
    }
}
