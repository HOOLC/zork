...
impl<M: InputModeKind> InputBaseState<M> {
    ...
    pub(super) fn move_vertical(
        ...
        if target_display_row == current_display_row {
            new_offset = if move_lines < 0 { 0 } else { self.text.len() };
        } else if let Some((preferred_x, column)) = was_preferred_column {
            ...
            if let Some(line) = last_layout.line(next_point.row) {
                ...
                let candidate = line_start_offset + column.min(max_line_len);
                // A byte column from another line can fall inside UTF-8 or an
                // extended grapheme when the target has not been shaped yet.
                let boundary = self.next_boundary(self.previous_boundary(candidate));
                new_offset = if boundary > candidate {
        ...
        self.cursor_line_end_affinity = self
            .display_map
            .offset_to_wrap_display_point(new_offset)
            .row
            > target_wrap_row;
    ...
    pub(super) fn select_vertical(
        ...
        let offset = self.cursor();
        let affinity = self.cursor_line_end_affinity;
        self.selected_range = selection;
        self.selection_reversed = reversed;
        self.select_to(offset, cx);
        self.cursor_line_end_affinity = affinity;
