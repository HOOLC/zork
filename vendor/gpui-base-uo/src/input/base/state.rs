...
impl<M: InputModeKind> InputBaseState<M> {
    ...
    /// Show the library's scrollbar chrome. Scrolling remains available when hidden.
    ...
    pub(super) fn start_of_line(&self) -> usize {
        ...
        if self.soft_wrap
            && !self.is_code_editor()
            && let Some(range) = self.textarea_visual_line_range()
        {
            return range.start;
        }
    ...
    fn textarea_visual_line_range(&self) -> Option<Range<usize>> {
        let row = self.text.offset_to_point(self.cursor()).row;
        let start = self.text.line_start_offset(row);
        let line = self.display_map.line(row)?;
        let local = self.cursor() - start;
        let index = line
            .wrapped_lines
            .partition_point(|range| {
                range.start < local || (!self.cursor_line_end_affinity && range.start == local)
            })
            .saturating_sub(1);
        line.wrapped_lines
            .get(index)
    ...
    pub(super) fn on_mouse_down(
        ...
        self.retain_mouse_line_affinity(offset, event.position);
    }

    fn retain_mouse_line_affinity(&mut self, offset: usize, position: Point<Pixels>) {
        if !self.is_code_editor()
            && let (Some(bounds), Some(layout)) = (self.last_bounds, self.last_layout.as_ref())
        {
            let row = ((position.y - bounds.origin.y) / layout.line_height)
                .floor()
                .max(0.) as usize;
            self.cursor_line_end_affinity =
                self.display_map.offset_to_wrap_display_point(offset).row > row;
            self.update_preferred_column();
        }
    }
    ...
    fn push_history(
        ...
        let intent = requested_intent.unwrap_or_else(|| {
            if (range.is_empty() || new_text.graphemes(true).count() == 1)
                && !new_text.is_empty()
                && !new_text.contains(['\n', '\r'])
            {
                EditIntent::Typing
    ...
    fn history_selection(&self) -> Selection {
    ...
    fn restore_history_selection(&mut self, selection: Selection) {
        self.selection_reversed = selection.start > selection.end;
        self.selected_range = Selection::new(
    ...
    /// Height of the wrapped text, independent of the current viewport.
    pub fn content_height(&self) -> Option<gpui::Pixels> {
        self.line_height()
            .map(|height| height * self.display_map.wrap_row_count() as f32)
    }
        ...
        if !self.is_single_line() {
            ...
            self.auto_scroll.set(scroll_delta, cx, |delta, state, cx| {
                ...
                if let Some(pos) = state.auto_scroll.last_drag_position {
                    ...
                    state.retain_mouse_line_affinity(offset, pos);
                }
            });
        }
    }
...
impl<M: InputModeKind> EntityInputHandler for InputBaseState<M> {
    ...
    fn replace_text_in_range(
        ...
        let selection_before = self.history_selection();
