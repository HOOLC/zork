...
impl DisplayMap {
    ...
    pub(crate) fn offset_to_wrap_display_point_with_affinity(
        ...
        let mut point = self.offset_to_wrap_display_point(offset);
        if upstream && point.local_row > 0 && point.column == 0 {
            point.row -= 1;
            point.local_row -= 1;
            point.column = offset - self.wrap_display_point_to_offset(point);
        }
        point
    }
