...
// A retained color target starts transparent. LCD blending's color-only
// writes cannot represent glyph coverage there, so retain grayscale alpha.
@fragment
fn fs_subpixel_sprite_retained(input: SubpixelSpriteOutput) -> SubpixelSpriteFragmentOutput {
    ...
    let coverage = input.color.a * dot(corrected, vec3<f32>(1.0 / 3.0));
    return SubpixelSpriteFragmentOutput(vec4<f32>(input.color.rgb, coverage), vec4<f32>(vec3<f32>(coverage), 1.0));
}
