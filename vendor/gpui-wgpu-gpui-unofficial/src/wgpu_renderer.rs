mod layers;
use layers::{LayerCache, LayerPipeline};
...
// A linear write needs at most a leading partial row, one rectangle of full
// rows, and a trailing partial row. Keep large uploads out of a per-row loop.
fn instance_texture_extent(width: u32, texel_offset: u64, texels: u64) -> Option<wgpu::Extent3d> {
    if texels == 0 {
        return None;
    }
    let column = texel_offset % u64::from(width);
    let rows = if column == 0 { texels / u64::from(width) } else { 0 };
    Some(wgpu::Extent3d {
        width: if rows > 0 { width } else { texels.min(u64::from(width) - column) as u32 },
...
struct PathSprite {
    ...
    texture_offset: Point<ScaledPixels>,
    pad: [u32; 2],
}
...
fn path_sprites(
    ...
    let bounds = if paths.last().map(|path| path.order) == Some(first.order) {
        paths.iter().map(Path::clipped_bounds).collect::<Vec<_>>()
    } else {
        vec![
            paths
                .iter()
    ...
    bounds
        .into_iter()
        .filter_map(|bounds| {
            let bounds = clip.map_or(bounds, |clip| bounds.intersect(&clip));
            (!bounds.is_empty()).then_some(PathSprite {
                bounds,
                texture_offset,
                pad: [0; 2],
            })
...
// One color/clip record per path, built before expanding its triangles.
#[derive(Default)]
struct PathData {
    words: Vec<u32>,
    attributes: Vec<u32>,
...
impl PathData {
    ...
    fn push(&mut self, color: Background, bounds: Bounds<ScaledPixels>, vertices: impl Iterator<Item = (Point<ScaledPixels>, Point<f32>)>) {
        let paint_offset = self.attributes.len() as u32;
        for bytes in [unsafe { WgpuRenderer::instance_bytes(std::slice::from_ref(&color)) },
                      unsafe { WgpuRenderer::instance_bytes(std::slice::from_ref(&bounds)) }] {
            self.attributes.extend(bytes.chunks_exact(4).map(|v| u32::from_ne_bytes(v.try_into().unwrap())));
        }
        for (xy, st) in vertices {
            self.words.extend_from_slice(&[xy.x.0.to_bits(), xy.y.0.to_bits(), st.x.to_bits(), st.y.to_bits(), paint_offset, 0]);
            self.vertices += 1;
        }
    }
    fn finish(&mut self) {
        self.words[0] = self.words.len() as u32;
        self.words.append(&mut self.attributes);
    }
}

// Separate atlas cells preserve the existing compositing semantics of each
// path batch while allowing one antialiased raster pass for the whole frame.
const MAX_PATH_ATLAS_TEXELS: u64 = 16 * 1024 * 1024;

fn path_texture_capacity(required: [u32; 2], retained: Option<[u32; 2]>, limit: u32) -> [u32; 2] {
    let retained = retained.unwrap_or([0; 2]);
    let grown = std::array::from_fn(|axis| {
        if retained[axis] >= required[axis] {
            retained[axis].min(limit)
        } else {
            required[axis]
                .max(retained[axis].saturating_add(retained[axis] / 2))
                .saturating_add(255)
                / 256
                * 256
        }
    ...
    // Keep spare capacity across small animation changes. Try single-axis
    // growth as well so slack never pushes the atlas over its memory budget.
    [grown, [required[0], grown[1]], [grown[0], required[1]]]
        .into_iter()
        .find(|&[width, height]| {
            width >= required[0]
                && height >= required[1]
                && u64::from(width) * u64::from(height) <= MAX_PATH_ATLAS_TEXELS
        })
        .unwrap_or(required)
}

struct PathAtlasBatch {
    range: Range<usize>,
    offset: Point<ScaledPixels>,
    visible: bool,
    bounds: Bounds<ScaledPixels>,
}
struct PathAtlasPlan {
    size: [u32; 2],
    batches: Vec<PathAtlasBatch>,
}
fn path_viewport(width: u32, height: u32) -> Bounds<ScaledPixels> {
    Bounds::new(
        Point::default(),
    ...
    for batch in scene.batches() {
        ...
        for path in &scene.paths[range.clone()] {
            ...
            if visible.size.width.0 > 0. && visible.size.height.0 > 0. {
                // Gradients and patterns use these bounds as their coordinate
                // basis; crop only solid paint without changing its color.
                let cell = if path.color.as_solid().is_some() { visible } else { clipped };
                bounds = Some(bounds.map_or(cell, |b| b.union(&cell)));
            }
        }
        let index = batches.len();
        batches.push(PathAtlasBatch {
            range,
        ...
        if let Some(bounds) = bounds {
            ...
            // Two clear pixels prevent adjacent cells from contributing through
            // bilinear filtering or multisample resolve at their edges.
            let width = bounds.right().0.ceil() - left + 4.;
            let height = bounds.bottom().0.ceil() - top + 4.;
            if !width.is_finite()
                || !height.is_finite()
                || width <= 0.
                || height <= 0.
            ...
            if area > MAX_PATH_ATLAS_TEXELS {
                return None;
            }
            widest = widest.max(width);
            cells.push((index, width, height, left, top));
        }
    }
    cells.sort_unstable_by_key(|cell| std::cmp::Reverse(cell.2));
    // Power-of-two widths waste entire columns for tall, viewport-sized
    // batches and can needlessly force a fallback during an animation. Try a
    // bounded set of row widths and retain the smallest fitting allocation.
    let square = (area as f64).sqrt();
    let mut widths: Vec<u32> = (1..=8).map(|n| widest.saturating_mul(n)).collect();
    widths.extend([0.5, 0.75, 1., 1.25, 1.5, 2.].map(|scale| (square * scale).ceil() as u32));
    for width in &mut widths {
        *width = (*width).max(widest).next_multiple_of(64).min(limit);
    }
    widths.sort_unstable();
    widths.dedup();
    let retained = retained.filter(|&[width, height]| {
        width >= widest
            && width <= limit
            && height <= limit
            && u64::from(width) * u64::from(height) <= MAX_PATH_ATLAS_TEXELS
    });
    if let Some([width, _]) = retained {
        widths.retain(|candidate| *candidate != width);
        widths.insert(0, width);
    }
    let mut best = None;
    let mut best_area = MAX_PATH_ATLAS_TEXELS + 1;
    for width in widths {
        let (mut x, mut y, mut row_height) = (0_u32, 0_u32, 0_u32);
        let mut offsets = Vec::with_capacity(cells.len());
        for &(index, cell_width, cell_height, left, top) in &cells {
            if x + cell_width > width {
                y += row_height;
                x = 0;
                row_height = 0;
            }
            if y + cell_height > limit {
                break;
            }
            offsets.push((
                index,
                gpui::point(
                    ScaledPixels(x as f32 + 2. - left),
                    ScaledPixels(y as f32 + 2. - top),
                ),
            ));
            x += cell_width;
        ...
        let height = (y + row_height).max(1).next_multiple_of(64).min(limit);
        if let Some(size) = retained {
            if width == size[0] && height <= size[1] {
                for (index, offset) in offsets {
                    batches[index].offset = offset;
                }
                return Some(PathAtlasPlan { size, batches });
            }
        }
        let packed_area = u64::from(width) * u64::from(height);
        if packed_area < best_area {
            best_area = packed_area;
            best = Some(([width, height], offsets));
        }
    }
    let (size, offsets) = best?;
    for (index, offset) in offsets {
        batches[index].offset = offset;
    }
    Some(PathAtlasPlan { size, batches })
}
...
struct WgpuPipelines {
    ...
    retained_subpixel_sprites: Option<wgpu::RenderPipeline>,
...
impl WgpuRenderer {
    ...
    fn new_internal(
        ...
        let layer_pipeline = LayerPipeline::new(&device, surface_config.format);
    ...
    fn create_pipelines(
        ...
        let subpixel_pipeline = |retained: bool| if let Some(subpixel_module) = &subpixel_shader_module {
            ...
            Some(create_pipeline(
                if retained { "retained_subpixel_sprites" } else { "subpixel_sprites" },
                "vs_subpixel_sprite",
                if retained { "fs_subpixel_sprite_retained" } else { "fs_subpixel_sprite" },
                &layouts.globals,
                &layouts.instances,
                Some(&layouts.texture),
                wgpu::PrimitiveTopology::TriangleStrip,
                &[Some(wgpu::ColorTargetState {
                    ...
                    write_mask: if retained { wgpu::ColorWrites::ALL } else { wgpu::ColorWrites::COLOR },
                })],
                1,
                subpixel_module,
            ))
        ...
        let subpixel_sprites = subpixel_pipeline(false);
        let retained_subpixel_sprites = subpixel_pipeline(true);
    ...
    fn ensure_intermediate_textures(&mut self, atlas: Option<&PathAtlasPlan>) {
        let format = self.surface_config.format;
        let mut size = atlas.map_or(
        ...
        if let Some([width, height]) = retained {
            let retained_area = u64::from(width) * u64::from(height);
            if width >= size[0]
                && height >= size[1]
                && (atlas.is_none() || retained_area <= MAX_PATH_ATLAS_TEXELS)
            {
                return;
            }
        }
        if atlas.is_some() {
            size = path_texture_capacity(size, retained, self.max_texture_size);
        }
    ...
    pub fn draw(&mut self, scene: &Scene) -> bool {
        ...
        if let Some(error) = last_error {
            self.resources_mut().layer_cache.entries.clear();
        ...
        let fallback;
        let scene = if layers::admitted(scene,
            [self.surface_config.width, self.surface_config.height],
            self.surface_config.format.block_copy_size(None).unwrap_or(4)) {
            scene
        } else {
            fallback = scene.without_retained_layers();
            &fallback
        };
        self.resources_mut().layer_cache.begin_frame(scene);
        let mut path_atlas = if cfg!(target_family = "wasm") && !self.uses_webgl_instance_data {
            plan_path_atlas(
                scene,
                path_viewport(self.surface_config.width, self.surface_config.height),
        ...
        // Retained subscenes rasterize in window coordinates before the root
        // atlas is populated. Use one compatible target/uniform cohort for
        // both, while keeping the root's path batches in a single pass.
        if !scene.retained_layers.is_empty() && let Some(plan) = &mut path_atlas {
            plan.size[0] = plan.size[0].max(self.surface_config.width);
            plan.size[1] = plan.size[1].max(self.surface_config.height);
        }
        self.ensure_intermediate_textures(path_atlas.as_ref());
        ...
        if let Err(error) = self.record_frame(scene, &frame_view, path_atlas.as_ref()) {
            log::error!("{error:#}");
            self.resources_mut().layer_cache.entries.clear();
    ...
    /// Observe completion of submitted WebGPU work, including device loss.
    /// The window uses this to bound queued frames without blocking input.
    #[cfg(target_family = "wasm")]
    pub fn on_frame_completed(&self, callback: impl FnOnce() + Send + 'static) -> bool {
        if self.uses_webgl_instance_data { return false; }
        self.resources().queue.on_submitted_work_done(callback);
        true
    }
    ...
    fn record_frame(
        ...
        self.record_scene(scene, frame_view, atlas, &mut encoder, &mut instance_offset, false)?;
        self.flush_instance_staging();
        self.resources()
            .queue
    ...
    fn record_scene(
        ...
        let retained_bindings =
            self.prepare_retained(&scene.retained_layers, encoder, instance_offset)?;
        // Retained color targets can use the same packed path pass as the
        // canvas. Reuse only the already allocated target size: changing its
        // uniforms or textures while recording nested layers would invalidate
        // earlier commands in this frame.
        let retained_atlas = if retained && atlas.is_none()
            && cfg!(target_family = "wasm") && !self.uses_webgl_instance_data {
            self.resources().path_intermediate_texture.as_ref().and_then(|texture| {
                let size = [texture.width(), texture.height()];
                plan_path_atlas(scene, path_viewport(self.surface_config.width, self.surface_config.height),
                    self.resources().device.limits().max_texture_dimension_2d, Some(size))
                    .filter(|plan| plan.size[0] <= size[0] && plan.size[1] <= size[1])
                    .map(|mut plan| { plan.size = size; plan })
            })
        } else { None };
        let atlas = atlas.or(retained_atlas.as_ref());
        ...
        if let Some(plan) = atlas {
            self.draw_paths_to_atlas(encoder, scene, plan, instance_offset)?;
        }
        let mut path_batch_index = 0;
        {
            ...
            for batch in scene.render_batches() {
                let batch = match batch {
                    gpui::SceneBatch::Primitives(batch) => batch,
                    gpui::SceneBatch::RetainedLayer(index) => {
                        if scene.retained_layers[index]
                            .effects
                            .iter()
                            .any(|effect| effect.opacity == 0.)
                        {
                            continue;
                        }
                        let resources = self.resources();
                        pass.set_pipeline(&resources.layer_pipeline.pipeline);
                        pass.set_bind_group(0, &retained_bindings[index], &[]);
                        pass.draw(0..4, 0..1);
                        continue;
                    }
                };
                match batch {
                    ...
                    PrimitiveBatch::Paths(range) => {
                        ...
                        if let Some(plan) = atlas {
                            let batch = &plan.batches[path_batch_index];
                            path_batch_index += 1;
                            if batch.visible {
                    ...
                    PrimitiveBatch::SubpixelSprites { texture_id, range } => {
                        ...
                        self.draw_sprites(
                            ...
                            texture_id,
                            (if retained { &resources.pipelines.retained_subpixel_sprites } else { &resources.pipelines.subpixel_sprites })
                                .as_ref()
    ...
    fn draw_paths_from_intermediate(
        ...
        let sprites = path_sprites(paths, texture_offset, clip);
        if sprites.is_empty() {
            return Ok(());
        }
    ...
    fn draw_paths_to_atlas(
        ...
        let viewport = path_viewport(self.surface_config.width, self.surface_config.height);
        let paths = plan.batches.iter().filter(|batch| batch.visible).flat_map(|batch| {
            scene.paths[batch.range.clone()].iter().filter_map(move |path| {
                let mut bounds = path.clipped_bounds();
                let visible = bounds.intersect(&viewport);
                if visible.is_empty() { return None; }
                if path.color.as_solid().is_some() { bounds = visible; }
                bounds.origin += batch.offset;
                Some((path, batch.offset, bounds))
            })
        });
        let (binding, count) = self.path_binding("path_rasterization_bind_group", instance_offset, paths)?;
        self.draw_path_binding(encoder, binding.as_ref(), count)
    }
    ...
    fn draw_paths_to_intermediate(
        ...
        let (binding, count) = self.path_binding("path_rasterization_bind_group", instance_offset,
            paths.iter().map(|path| (path, Point::default(), path.clipped_bounds())))?;
        self.draw_path_binding(encoder, binding.as_ref(), count)
    }

    fn draw_path_binding(
        &self,
        encoder: &mut wgpu::CommandEncoder,
        vertex_binding: Option<&InstanceBinding>,
        count: u32,
    ) -> Result<bool> {
        let resources = self.resources();
...

        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                ...
                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                    ...
                    ops: wgpu::Operations {
                        load: wgpu::LoadOp::Clear(wgpu::Color::TRANSPARENT),
                        // Only the resolved image is sampled by subsequent passes.
                        // Storing the multisampled attachment needlessly spills
                        // full-viewport samples to memory on tile-based GPUs.
            ...
            if let Some(binding) = vertex_binding {
                pass.set_pipeline(&resources.pipelines.path_rasterization);
                pass.set_bind_group(0, &resources.path_globals_bind_group, &[]);
                pass.set_bind_group(1, &binding.bind_group, &[]);
                pass.draw(binding.first_instance..binding.first_instance + count, 0..1);
            }
        }
    ...
    fn path_binding<'a>(
        ...
        label: &str,
        instance_offset: &mut u64,
        paths: impl Iterator<Item = (&'a Path<ScaledPixels>, Point<ScaledPixels>, Bounds<ScaledPixels>)>,
    ) -> Result<(Option<InstanceBinding>, u32)> {
        let viewport = path_viewport(self.surface_config.width, self.surface_config.height);
        if self.uses_webgl_instance_data {
            let mut vertices = Vec::new();
            for (path, offset, bounds) in paths {
                vertices.extend(path.visible_triangles_in(viewport).flatten().map(|v| PathRasterizationVertex {
                    xy_position: v.xy_position + offset, st_position: v.st_position, color: path.color, bounds,
                }));
            }
            let count = vertices.len() as u32;
            return if count == 0 { Ok((None, 0)) }
            else { self.write_instance_binding(label, instance_offset, &vertices).map(|binding| (Some(binding), count)) };
        }
        let mut data = std::mem::take(&mut self.path_data);
        data.reset();
        for (path, offset, bounds) in paths {
            data.push(path.color, bounds, path.visible_triangles_in(viewport).flatten().map(|v| {
                (if offset == Point::default() { v.xy_position } else { v.xy_position + offset }, v.st_position)
            }));
        }
        let count = data.vertices;
        data.finish();
        let result = if count == 0 { Ok((None, 0)) }
            else { self.write_instance_binding(label, instance_offset, &data.words).map(|binding| (Some(binding), count)) };
        self.path_data = data;
        result
    }
    ...
    fn write_instance_binding<T>(
        ...
        if offset + allocation_size > self.instance_data_capacity {
            // Earlier bindings still point at the old allocation. Commit their
            // bytes before replacing it, then begin a batch for the new one.
            self.flush_instance_staging();
        ...
        *instance_offset = offset + allocation_size;
        ...
        if !data.is_empty() {
            ...
            self.instance_staging.resize(start + data.len(), 0);
            self.instance_staging[start..].copy_from_slice(data);
        }
        let resources = self.resources();
    ...
    /// Batch an allocation into one buffer upload or a few texture rectangles,
    /// preserving primitive order without a transaction for every path/row.
    fn flush_instance_staging(&mut self) {
        if !self.instance_staging.is_empty() {
            let resources = self.resources();
            match &resources.instance_data {
                InstanceData::Storage(buffer) => {
                    resources.queue.write_buffer(buffer, 0, &self.instance_staging);
                }
                InstanceData::Texture { .. } => {
                    Self::write_instance_texture(resources, 0, &self.instance_staging);
                }
            }
            self.instance_staging.clear();
        }
    ...
    fn write_instance_texture(resources: &WgpuResources, offset: u64, data: &[u8]) {
        ...
        while byte_offset < data.len() {
            ...
            if let Some(extent) = instance_texture_extent(*width, texel_offset, complete_texels) {
                ...
                resources.queue.write_texture(
                    ...
                    wgpu::TexelCopyBufferLayout {
                        offset: 0,
                        bytes_per_row: Some(extent.width * INSTANCE_TEXTURE_TEXEL_SIZE as u32),
                        rows_per_image: None,
                    },
                    extent,
                );
...
mod tests {
    ...
    #[test]
    fn instance_texture_uploads_cover_exact_span_in_at_most_three_rectangles() {
        for width in [1, 4, 7, 2048] {
            let width64 = u64::from(width);
            for offset in [0, 1, width64 - 1, width64, width64 + 1] {
                for count in [1, width64, width64 + 1, width64 * 4 + 3] {
                    let mut texture = vec![false; (offset + count + width64) as usize];
                    let mut next = offset;
                    let mut uploads = 0;
                    while let Some(extent) = instance_texture_extent(width, next, offset + count - next) {
                        assert!(next % width64 + u64::from(extent.width) <= width64);
                        assert!(extent.height == 1 || (next % width64 == 0 && extent.width == width));
                        for row in 0..u64::from(extent.height) {
                            for column in 0..u64::from(extent.width) {
                                let slot = &mut texture[(next + row * width64 + column) as usize];
                                assert!(!*slot, "upload regions overlap");
                                *slot = true;
                            }
                        }
                        next += u64::from(extent.width) * u64::from(extent.height);
                        uploads += 1;
                    }
                    assert!(uploads <= 3);
                    assert!(texture.iter().enumerate().all(|(index, &written)|
                        written == (offset..offset + count).contains(&(index as u64))));
                }
            }
        }
        let full_rows = instance_texture_extent(2048, 0, 2048 * 100).unwrap();
        assert_eq!((full_rows.width, full_rows.height), (2048, 100));
    }
    ...
    #[test]
    fn packed_paths_round_trip_all_color_and_clip_bits_with_shared_attributes() {
        let original = PathRasterizationVertex {
            xy_position: gpui::point(ScaledPixels(-0.), ScaledPixels(13.25)),
            st_position: gpui::point(0.5, 1.),
            color: gpui::linear_gradient(27., gpui::linear_color_stop(gpui::rgb(0x448899), 0.2), gpui::linear_color_stop(gpui::rgba(0xff001188), 0.8)),
            bounds: Bounds::new(gpui::point(ScaledPixels(-0.), ScaledPixels(8.)), gpui::size(ScaledPixels(100.), ScaledPixels(200.))),
        };
        let mut vertices = vec![original; 96];
        vertices[30].bounds.origin.x.0 = 0.;
        vertices[60].color = gpui::checkerboard(gpui::rgb(0x889944), 8.);
        let mut data = PathData::default();
        data.reset();
        for range in [0..30, 30..31, 31..60, 60..61, 61..96] {
            let first = &vertices[range.start];
            data.push(first.color, first.bounds, vertices[range].iter().map(|v| (v.xy_position, v.st_position)));
        }
        data.finish();
        let words = &data.words;
        assert!(words.len() * 4 < vertices.len() * std::mem::size_of::<PathRasterizationVertex>() / 3);
        for (index, vertex) in vertices.iter().enumerate() {
            let expected = unsafe { WgpuRenderer::instance_bytes(std::slice::from_ref(vertex)) };
            let mut decoded = Vec::new();
            let base = 1 + index * 6;
            let offset = (words[0] + words[base + 4]) as usize;
            for word in words[base..base + 4].iter().chain(words[offset..offset + 22].iter()) {
                decoded.extend(word.to_ne_bytes());
            }
            assert_eq!(decoded, expected);
        }
        data.reset();
        assert_eq!(data.vertices, 0);
        assert_eq!(data.words, [0]);
        assert!(data.attributes.is_empty());
    }

    fn atlas_test_scene(rectangles: &[[f32; 4]]) -> Scene {
        let mut scene = Scene::default();
        for (index, &[x, y, width, height]) in rectangles.iter().enumerate() {
            let mut path = Path::new(gpui::point(gpui::px(x), gpui::px(y)));
            path.bounds = Bounds::new(
                path.bounds.origin,
                gpui::size(gpui::px(width), gpui::px(height)),
            );
            path.content_mask.bounds = path.bounds;
            path.order = (index * 2) as u32;
            scene.paths.push(path.scale(1.));
            // Interleaved content separates batches with overlapping screen bounds.
    ...
    #[test]
    fn atlas_separates_overlapping_batches_and_preserves_pixel_alignment() {
        let scene = atlas_test_scene(&[[-4.2, 2.7, 60.3, 40.1], [10., 7., 30., 26.]]);
        let plan = plan_path_atlas(&scene, path_viewport(100, 100), 256, None).unwrap();
        ...
        let placed: Vec<_> = plan
            ...
            .map(|batch| {
                ...
                // An integer translation keeps the same MSAA coverage as screen-space rasterization.
                assert_eq!(batch.offset.x.0.fract(), 0.);
                assert_eq!(batch.offset.y.0.fract(), 0.);
                let mut bounds = batch.bounds;
                bounds.origin += batch.offset;
                assert!(bounds.origin.x.0 >= 2. && bounds.origin.y.0 >= 2.);
                assert!(bounds.right().0 <= plan.size[0] as f32 - 2.);
                assert!(bounds.bottom().0 <= plan.size[1] as f32 - 2.);
                bounds
            })
            .collect();
        assert!(placed[0].intersect(&placed[1]).is_empty());
    }

    #[test]
    fn atlas_culls_hidden_batches_and_falls_back_within_device_and_memory_limits() {
        let hidden = atlas_test_scene(&[[200., 200., 20., 20.]]);
        let plan = plan_path_atlas(&hidden, path_viewport(100, 100), 256, None).unwrap();
        assert!(!plan.batches[0].visible);
        let oversized = atlas_test_scene(&[[0., 0., 256., 20.]]);
        assert!(plan_path_atlas(&oversized, path_viewport(256, 100), 256, None).is_none());
        let crowded = atlas_test_scene(&[[0., 0., 2000., 1500.]; 7]);
        assert!(plan_path_atlas(&crowded, path_viewport(3000, 2000), 4096, None).is_none());
    }

    #[test]
    fn atlas_keeps_scrolled_geometry_within_the_visible_target() {
        let scene = atlas_test_scene(&[[-4000., -4000., 10000., 10000.]]);
        let viewport = path_viewport(100, 100);
        let plan = plan_path_atlas(&scene, viewport, 256, None).unwrap();
        assert_eq!(plan.batches[0].bounds, viewport);
        assert!(plan.size[0] <= 128 && plan.size[1] <= 128);
        assert_eq!(plan.batches[0].offset, gpui::point(ScaledPixels(2.), ScaledPixels(2.)));
    }

    #[test]
    fn atlas_preserves_pattern_coordinate_bounds_when_partially_offscreen() {
        let mut scene = atlas_test_scene(&[[-40., -40., 150., 150.]]);
        scene.paths[0].color = gpui::checkerboard(gpui::rgb(0x445566), 8.);
        let plan = plan_path_atlas(&scene, path_viewport(100, 100), 256, None).unwrap();
        assert_eq!(plan.batches[0].bounds, scene.paths[0].clipped_bounds());
    }

    #[test]
    fn atlas_packs_retina_batches_without_power_of_two_overallocation() {
        let scene = atlas_test_scene(&[[0., 0., 1476., 1674.]; 6]);
        let plan = plan_path_atlas(&scene, path_viewport(1800, 1900), 8192, None).unwrap();
        assert_eq!(plan.batches.len(), 6);
        assert!(u64::from(plan.size[0]) * u64::from(plan.size[1]) <= MAX_PATH_ATLAS_TEXELS);
    }

    #[test]
    fn atlas_reuses_storage_when_animated_bounds_shrink() {
        let viewport = path_viewport(800, 800);
        let large = atlas_test_scene(&[[0., 0., 700., 600.]; 4]);
        let plan = plan_path_atlas(&large, viewport, 4096, None).unwrap();
        let small = atlas_test_scene(&[[0., 0., 580., 540.]; 4]);
        let next = plan_path_atlas(&small, viewport, 4096, Some(plan.size)).unwrap();
        assert_eq!(
            next.size, plan.size,
            "minor animation changes must not reallocate GPU attachments"
        );
    }

    #[test]
    fn atlas_capacity_absorbs_first_animation_growth_without_reallocation() {
        let mut capacity = [1536, 5888];
        let mut allocations = 0;
        for height in [5952, 6016, 6144, 6272, 6336, 6400, 6464] {
            let next = path_texture_capacity([1536, height], Some(capacity), 8192);
            allocations += usize::from(next != capacity);
            capacity = next;
            assert!(capacity[0] >= 1536 && capacity[1] >= height);
        }
        assert_eq!(allocations, 1);
        assert_eq!(capacity, [1536, 8192]);
    }

    #[test]
    fn atlas_capacity_slack_respects_device_and_memory_limits() {
        for (required, retained, limit) in [
            ([4000, 4000], Some([3968, 3968]), 8192),
            ([4096, 4096], Some([4096, 4000]), 4096),
            ([8000, 2000], None, 8192),
            ([6000, 2500], Some([8192, 1900]), 8192),
        ] {
            let size = path_texture_capacity(required, retained, limit);
            assert!(size[0] >= required[0] && size[1] >= required[1]);
            assert!(size[0] <= limit && size[1] <= limit);
            assert!(u64::from(size[0]) * u64::from(size[1]) <= MAX_PATH_ATLAS_TEXELS);
        }
    }

    #[test]
    fn atlas_sprites_cannot_sample_neighboring_cells_for_hidden_paths() {
        let clip = path_viewport(100, 100);
        for same_order in [false, true] {
            let mut scene = atlas_test_scene(&[[0., 0., 100., 100.], [500., 0., 100., 100.]]);
            if same_order {
                scene.paths[1].order = scene.paths[0].order;
            }
            let sprites = path_sprites(&scene.paths, Point::default(), Some(clip));
            assert_eq!(sprites.len(), 1);
            assert_eq!(sprites[0].bounds, clip);
        }
    }
}
