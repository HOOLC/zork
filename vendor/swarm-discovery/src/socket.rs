...
impl Sockets {
    ...
    pub async fn send_msg(&self, msg: &Message, mode: Mode) {
        // The owner recreates failed sockets under its retry policy. Incoming
        // queries must not keep a broken interface sending and logging.
        if self.failure.borrow().is_some() { return; }
    ...
    async fn send_msg_multi_interface_v4(&self, bytes: &[u8], msg: &Message) {
        ...
        for (addr, socket) in interfaces.iter() {
            if let Err(e) = socket.send_to(bytes, (MDNS_IPV4, MDNS_PORT)).await {
                self.fail(format!("mDNS interface {addr}: {e}"));
            } else {
                tracing::debug!(
                    addr = %addr,
                    q = msg.queries.len(),
        ...
        if sent_count == 0 {
            self.fail("failed to send mDNS on every IPv4 interface");
        }
    }
}
