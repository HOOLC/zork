...
impl Discoverer {
    ...
    pub fn spawn(self, handle: &Handle) -> Result<DropGuard, SpawnError> {
        ...
        let failure = sockets.failure.subscribe();
        ...
        let (finished, completion) = tokio::sync::watch::channel(());
        let SupervisionRef { me, handle } = rt.spawn_actor("guardian", move |ctx| async move {
            let _finished = finished;
            guardian::guardian(ctx, self, sockets, service_name).await
        });
...
impl DropGuard {
    /// Wait for a send failure or an unexpected discovery task exit.
    pub async fn failed(&mut self) -> String {
        if let Some(error) = self.failure.borrow().clone() { return error; }
        tokio::select! {
            _ = self.completion.changed() => "discovery stopped".into(),
            _ = self.failure.changed() => self.failure.borrow().clone().unwrap_or_else(|| "discovery stopped".into()),
        }
    }

    /// Stop discovery and wait for the owned guardian to release its children.
