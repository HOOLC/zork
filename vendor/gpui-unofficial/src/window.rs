...
mod prompts;
mod paint_region;
pub use paint_region::PaintRegion;
...
impl FocusId {
    ...
    pub fn is_focused(&self, window: &Window) -> bool {
        let result = window.focus == Some(*self);
        window.observe_focus_query(FocusQuery::Is(*self), FocusValue::Flag(result));
        result
    }
    ...
    pub fn contains_focused(&self, window: &Window, cx: &App) -> bool {
        let result = window.current_focus(cx).is_some_and(|focused| self.contains(focused.id, window));
        window.observe_focus_query(FocusQuery::Contains(*self), FocusValue::Flag(result));
        result
    }
    ...
    pub fn within_focused(&self, window: &Window, cx: &App) -> bool {
        let result = window.current_focus(cx).is_some_and(|focused| focused.id.contains(*self, window));
        window.observe_focus_query(FocusQuery::Within(*self), FocusValue::Flag(result));
        result
    }
...
impl HitboxId {
    ...
    pub fn is_hovered(self, window: &Window) -> bool {
        ...
        if window.captured_hitbox == Some(self) {
            return crate::InteractionGate::allows(window.captured_interaction_gates.as_deref());
        }
        self.hit_test(window) && !window.last_input_was_keyboard()
    }
...
pub(crate) struct DeferredDraw {
    ...
    interaction_gates: Option<Arc<[crate::InteractionGate]>>,
...
pub(crate) struct Frame {
    ...
    pub(crate) scene: paint_region::FrameScene,
...
pub(crate) type TextLayoutCapture = Rc<RefCell<Vec<Range<LineLayoutIndex>>>>;
...
impl Frame {
    ...
    pub(crate) fn hit_test(&self, position: Point<Pixels>) -> HitTest {
        ...
        for hitbox in self.hitboxes.iter().rev() {
            if !crate::InteractionGate::allows(hitbox.interaction_gates.as_deref()) { continue; }
...
impl FocusQuery {
    fn evaluate(self, focus: Option<FocusId>, tree: &DispatchTree) -> FocusValue {
        match self {
            Self::Any => FocusValue::Focus(focus),
            Self::Is(id) => FocusValue::Flag(focus == Some(id)),
            Self::Contains(id) => {
                FocusValue::Flag(focus.is_some_and(|focus| tree.focus_contains(id, focus)))
            }
            Self::Within(id) => {
                FocusValue::Flag(focus.is_some_and(|focus| tree.focus_contains(focus, id)))
            }
        }
    }
}
...
struct PresentationDependencies {
    generation: u64,
    modality: bool,
    focus: FxHashMap<FocusQuery, SmallVec<[FocusValue; 1]>>,
}
...
pub struct Window {
    ...
    text_layout_captures: Vec<TextLayoutCapture>,
    pub(crate) element_offset_stack: Vec<Point<Pixels>>,
    pub(crate) element_opacity: f32,
    gpu_layers_enabled: bool,
    pub(crate) interaction_gates: Option<Arc<[crate::InteractionGate]>>,
    captured_interaction_gates: Option<Arc<[crate::InteractionGate]>>,
    retained_paint_clock: u64,
    retained_paint: FxHashMap<u64, (u64, std::sync::Arc<crate::RetainedContent>)>,
    ...
    presentation_dependencies: RefCell<FxHashMap<EntityId, PresentationDependencies>>,
...
impl Window {
    pub(crate) fn new(
        ...
        let gpu_layers_enabled = sprite_atlas.supports_retained_paint();
        Ok(Window {
            handle,
            invalidator,
            removed: false,
            ...
            text_layout_captures: Vec::new(),
            element_offset_stack: Vec::new(),
            content_mask_stack: Vec::new(),
            element_opacity: 1.0,
            gpu_layers_enabled,
            interaction_gates: None,
            captured_interaction_gates: None,
            retained_paint_clock: 0,
            retained_paint: FxHashMap::default(),
            ...
            presentation_dependencies: RefCell::default(),
            refreshing: false,
...
impl Window {
    /// Whether this view needs fresh content/layout in the current draw.
    pub fn view_needs_render(&self, view: EntityId) -> bool {
        self.refreshing || self.dirty_views.contains(&view)
    }
    ...
    pub fn focused(&self, cx: &App) -> Option<FocusHandle> {
        let focus=self.current_focus(cx);
        self.observe_focus_query(FocusQuery::Any, FocusValue::Focus(focus.as_ref().map(|focus|focus.id)));
        focus
    }

    fn current_focus(&self, cx: &App) -> Option<FocusHandle> {
        self.focus
    ...
    pub fn focus(&mut self, handle: &FocusHandle, cx: &mut App) {
        ...
        self.refresh_focus_change(previous);
    }
    ...
    pub fn render_to_image(&self) -> anyhow::Result<image::RgbaImage> {
        self.platform_window
            .render_to_image(self.presentation_scene.as_ref().unwrap_or(&self.rendered_frame.scene))
    }
    ...
    pub fn capture_pointer(&mut self, hitbox_id: HitboxId) {
        ...
        self.captured_interaction_gates = self.rendered_frame.hitboxes.iter().find(|hitbox| hitbox.id == hitbox_id)
            .and_then(|hitbox| hitbox.interaction_gates.clone());
    }
    ...
    pub fn last_input_was_keyboard(&self) -> bool {
        if let Some(view) = self.rendered_entity_stack.last() {
    ...
    fn observe_focus_query(&self, query: FocusQuery, value: FocusValue) {
        if let Some(view) = self.rendered_entity_stack.last() {
            let mut dependencies = self.presentation_dependencies.borrow_mut();
    ...
    fn changed_focus_dependents(&self, focus: Option<FocusId>, only_within: bool) -> Vec<EntityId> {
        self.presentation_dependencies
            .borrow()
            .iter()
            .filter(|(_, dependency)| {
                dependency.focus.iter().any(|(query, values)| {
                    if only_within && !matches!(query, FocusQuery::Within(_)) { return false; }
                    let current = query.evaluate(focus, &self.rendered_frame.dispatch_tree);
    ...
    fn refresh_focus_change(&mut self, previous: Option<FocusId>) {
        if previous == self.focus {
            return;
        }
        // The end-of-draw focus phase owns forwarding and lost-focus recovery.
        // Do not schedule from those callbacks: a recovery handle may have no
        // painted node, and repeatedly redrawing it would loop focus-lost.
        if self.invalidator.inner.borrow().draw_phase == DrawPhase::Focus {
            return;
        }
        let full = self.focus.is_some_and(|focus| {
            self.rendered_frame
                .dispatch_tree
                .focusable_node_id(focus)
                .is_none()
        });
        let affected = self.changed_focus_dependents(self.focus, false);
        if self.invalidator.not_drawing() {
            if full {
                self.refresh();
            } else {
            ...
            self.on_next_frame(move |window, cx| {
                // A focus target introduced by this draw was absent from the
                // previous tree. Recheck after mounting it instead of carrying
                // that stale absence into a full-window refresh next frame.
                if window.focus.is_some_and(|focus| window.rendered_frame
                    .dispatch_tree.focusable_node_id(focus).is_none()) {
                    window.refresh();
                } else {
                    for view in window.changed_focus_dependents(window.focus, false) {
                        window.invalidator.invalidate_view(view, cx);
                    }
                }
            });
        }
    ...
    pub(crate) fn begin_view_render(&self, view: EntityId) {
        let mut observers = self.presentation_dependencies.borrow_mut();
        let entry = observers.entry(view).or_default();
        if entry.generation != self.next_frame.generation {
            entry.generation = self.next_frame.generation;
    ...
    pub fn draw(&mut self, cx: &mut App) -> ArenaClearNeeded {
        ...
        let focus_path_changed = previous_focus_path != current_focus_path;
        if focus_path_changed || previous_window_active != current_window_active
        {
        ...
        // panel). The focus phase defers invalidation until traversal completes;
        ...
        if self.focus != focus_before_listeners {
            self.refresh_focus_change(focus_before_listeners);
        } else if self.focus.is_none_or(|focus| self.rendered_frame.dispatch_tree.focusable_node_id(focus).is_some()) {
            // A focused node can move between containers without changing its
            // handle. Queries read the previous complete tree during render;
            // refresh only cached results changed by the completed new tree.
            let focus=self.current_focus(cx).map(|focus|focus.id);
            let affected=self.changed_focus_dependents(focus, !focus_path_changed && self.focus_generation == focus_generation_at_start);
            if !affected.is_empty() {
                self.on_next_frame(move |window,cx| {
                    for view in affected { window.invalidator.invalidate_view(view,cx); }
                });
            }
        }
    ...
    fn invalidate_entities(&mut self) {
        self.presentation_dependencies.borrow_mut().retain(|view, _| {
            self.rendered_frame.dispatch_tree.view_path_reversed(*view).next().is_some()
        });
    ...
    /// Retire display-link requests for a dedicated benchmark window. The
    /// benchmark drives normal input, animation and dirty rendering itself.
    /// This window must be closed at the end of the benchmark.
    #[cfg(feature = "bench")]
    pub fn prepare_benchmark_frames(&mut self) {
        self.platform_window.on_request_frame(Box::new(|_| {}));
    }

    /// Whether another animation frame is requested after the current one.
    #[cfg(feature = "bench")]
    pub fn has_animation_frames(&self) -> bool {
        !self.next_frame_callbacks.borrow().is_empty()
    }

    /// Counts of retained input metadata for native benchmark diagnostics:
    /// nodes, hitboxes, listeners, input handlers, states and state accesses.
    #[cfg(feature = "bench")]
    pub fn benchmark_input_counts(&self) -> [usize; 6] {
        let frame = &self.rendered_frame;
        [frame.dispatch_tree.len(), frame.hitboxes.len(), frame.mouse_listeners.len(),
            frame.input_handlers.len(), frame.element_states.len(), frame.accessed_element_states.len()]
    }

    /// Draw dirty UI and observe this frame's GPU completion. Animation
    /// callbacks and input run in a preceding App update so their notifications
    /// have been applied. No pixels are read back and clean frames are omitted.
    #[cfg(feature = "bench")]
    pub fn benchmark_frame(&mut self, cx: &mut App, offscreen: bool) -> Result<Option<oneshot::Receiver<crate::FrameCompletion>>> {
        if self.invalidator.is_dirty() {
            self.draw(cx).clear(cx);
        }
        if !self.needs_present.get() { return Ok(None); }
        let (sender, receiver) = oneshot::channel();
        let observed = crate::frame_completion::observe(Box::new(move |frame| {
            let _ = sender.send(frame);
        }), offscreen, || self.present());
        anyhow::ensure!(observed, "renderer does not support GPU completion measurement");
        Ok(Some(receiver))
    }
    ...
    fn draw_roots(&mut self, cx: &mut App) {
        let first_frame = self.rendered_frame.generation == 0;
        if first_frame { crate::observe_startup("gpui.first_frame_begin"); }
        ...
        if first_frame { crate::observe_startup("gpui.first_frame_layout_requested"); }
        self.layout_engine
            .as_mut()
            .unwrap()
        ...
        if first_frame { crate::observe_startup("gpui.first_frame_prepainted"); }
        ...
        if first_frame { crate::observe_startup("gpui.first_frame_painted"); }
    ...
    fn prepaint_deferred_draws(&mut self, cx: &mut App) {
        ...
        loop {
            ...
            for deferred_draw_ix in traversal_order {
                self.deferred_draw_priority = self.next_frame.deferred_draws[deferred_draw_ix].priority;
                let (element, parent_node, current_view, rem_size, absolute_offset, prepaint_range) = {
                    ...
                    self.interaction_gates=deferred_draw.interaction_gates.clone();
                    (
    ...
    fn paint_deferred_draws(&mut self, cx: &mut App) {
        ...
        for deferred_draw_ix in traversal_order {
            ...
            self.deferred_draw_priority = deferred_draw.priority;
            self.element_id_stack
                .clone_from(&deferred_draw.element_id_stack);
            self.next_frame
                .dispatch_tree
    ...
    pub(crate) fn capture_layout_text<R>(
        &mut self,
        capture: TextLayoutCapture,
        callback: impl FnOnce(&mut Self) -> R,
    ) -> R {
        self.text_layout_captures.push(capture);
        let result = callback(self);
        self.text_layout_captures.pop();
        result
    }
        ...
        self.next_frame.deferred_draws.extend(
            self.rendered_frame.deferred_draws
                ...
                .map(|deferred_draw| DeferredDraw {
                    ...
                    interaction_gates: deferred_draw.interaction_gates.clone(),
    ...
    pub(crate) fn reuse_paint(&mut self, range: Range<PaintIndex>) {
        ...
        self.text_system
            .reuse_layouts(range.start.line_layout_index..range.end.line_layout_index);
        let retained = self.gpu_layers_enabled();
        self.next_frame.scene.replay_with_retained(
    ...
    /// The live input scopes inherited by the current subtree.
    pub fn interaction_gates(&self) -> Option<Arc<[crate::InteractionGate]>> {
        self.interaction_gates.clone()
    }

    /// Execute a layout, prepaint or paint operation under a stable input gate.
    pub fn with_interaction_gate<R>(
        &mut self,
        gate: crate::InteractionGate,
        f: impl FnOnce(&mut Self) -> R,
    ) -> R {
        let previous = self.interaction_gates.take();
        let mut gates = previous.as_deref().unwrap_or_default().to_vec();
        gates.push(gate);
        self.interaction_gates = Some(gates.into());
        let result = f(self);
        self.interaction_gates = previous;
        result
    }
    ...
    /// Enable retained texture layers when the platform renderer supports
    /// Scene::render_batches. Other renderers keep the ordinary paint path.
    pub fn enable_gpu_layers(&mut self, enabled: bool) {
        let enabled = enabled && self.sprite_atlas.supports_retained_paint();
        if self.gpu_layers_enabled != enabled {
            self.gpu_layers_enabled = enabled;
            self.refresh();
        }
    }

    /// Whether this viewport fits the bounded offscreen-layer allocation.
    pub fn gpu_layers_enabled(&self) -> bool {
        let size = self.viewport_size();
        self.gpu_layers_enabled
            && self.next_frame.scene.can_retain()
            && size.width.as_f32() * size.height.as_f32() * self.scale_factor().powi(2) * 4.
                <= 16. * 1024. * 1024.
    }

    /// Whether a new contour-mask effect fits the retained compositor. Callers
    /// use their ordinary clipping path when an ancestor already has an effect.
    pub fn gpu_mask_layers_enabled(&self) -> bool {
        self.gpu_layers_enabled() && self.next_frame.scene.can_mask_retain()
    }

    /// Retain a scene fragment independently of input/event replay. The small
    /// LRU also preserves texture identity over short hidden/reopened periods.
    pub fn with_retained_paint<R>(
        &mut self,
        key: u64,
        bounds: Bounds<Pixels>,
        f: impl FnOnce(&mut Self) -> R,
        ...
        self.invalidator.debug_assert_paint();
        if !self.gpu_layers_enabled() {
            return f(self);
        }
        self.next_frame
            .scene
            .begin_retained(key, bounds.scale(self.scale_factor()));
        let result = f(self);
        let content = self
            .next_frame
            .scene
            .end_retained(self.retained_paint.get(&key).map(|(_, v)| v));
        self.retained_paint_clock += 1;
        self.retained_paint
            .insert(key, (self.retained_paint_clock, content));
        while self.retained_paint.len() > 8 {
            let oldest = *self
                .retained_paint
                .iter()
                ...
                .unwrap()
                .0;
            self.retained_paint.remove(&oldest);
        }
        result
    }

    /// Capture drawing without presenting it. The element is still visited
    /// once, so input handlers, focus and accessibility keep their owner.
    pub fn capture_paint_snapshot<R>(
        &mut self,
        key: u64,
        bounds: Bounds<Pixels>,
        previous: Option<&crate::PaintSnapshot>,
        f: impl FnOnce(&mut Self) -> R,
    ) -> (R, crate::PaintSnapshot) {
        self.invalidator.debug_assert_paint();
        self.next_frame.scene.begin_retained(key, bounds.scale(self.scale_factor()));
        let result = f(self);
        let content = self.next_frame.scene.end_snapshot(previous.map(|p| &p.0));
        (result, crate::PaintSnapshot(content))
    }

    /// Present captured drawing without replaying any input or element state.
    ...
    /// text rendering as other retained content.
    pub fn paint_retained_snapshot(&mut self, snapshot: &crate::PaintSnapshot) {
        self.invalidator.debug_assert_paint();
        if self.gpu_layers_enabled() {
            self.next_frame.scene.record_retained_content(snapshot.0.clone());
        } else {
            self.next_frame.scene.paint_snapshot(snapshot);
        }
    }

    /// Drawing layer of the current layout/paint traversal. Sources retain
    /// this destination identity independently of their screen coordinates.
    pub fn drawing_priority(&self) -> usize { self.deferred_draw_priority }

    /// Sequence of frames submitted to the platform. A UI paint can precede
    /// submission, so it alone cannot acknowledge a presentation handoff.
    pub fn presentation_sequence(&self) -> u64 { self.presentation_sequence }

    /// Replay a transferred picture without flattening its existing retained
    /// layers. Offset belongs to the destination's layout, not its animation.
    pub fn paint_snapshot_at(&mut self, snapshot: &crate::PaintSnapshot, offset: Point<Pixels>) {
        self.invalidator.debug_assert_paint();
        let bounds = snapshot.bounds().map(|value| px(value.0 / self.scale_factor()));
        let destination = Bounds::new(bounds.origin + offset, bounds.size);
        self.paint_barrier(destination);
        if offset == point(px(0.), px(0.)) {
            self.next_frame.scene.replay(0..snapshot.0.scene.len(), &snapshot.0.scene);
        } else {
            let mut effect = crate::scene::PaintEffect::new(
                point(ScaledPixels(0.), ScaledPixels(0.)), 1.,
                vec![self.content_mask().bounds.scale(self.scale_factor())],
            );
            effect.translation = offset.scale(self.scale_factor());
            self.next_frame.scene.push_paint_effect(effect);
            self.next_frame.scene.replay(0..snapshot.0.scene.len(), &snapshot.0.scene);
            self.next_frame.scene.pop_paint_effect();
        }
        self.paint_barrier(destination);
    }

    /// End a composition phase even inside an enclosing paint layer. Later
    /// primitive batches must remain above everything already painted here.
    pub fn paint_barrier(&mut self, bounds: Bounds<Pixels>) {
        self.invalidator.debug_assert_paint();
        self.next_frame.scene.paint_barrier(bounds.scale(self.scale_factor()));
    }

    /// Invalidate a view whose presentation ownership changes during render.
    /// Mark it immediately so a later cached child in this frame sees the change.
    pub fn invalidate_presentation(&mut self, view: EntityId, cx: &mut App) {
        self.mark_view_dirty(view);
        // The current view's source elements have not been prepainted yet;
        // they consume the new ownership in this draw. Other owners may have
        // already been visited and still need another invalidation.
        if self.rendered_entity_stack.last() != Some(&view)
            || self.invalidator.inner.borrow().draw_phase != DrawPhase::Prepaint
        {
            self.invalidator.invalidate_view(view, cx);
        }
    }

    /// Composite a retained subtree through the original path on the GPU.
    /// Geometry is in window coordinates; only content is scaled, not the mask.
    /// Returns None without painting when the caller must use its CPU fallback.
    pub fn with_masked_retained_paint<R>(
        &mut self,
        key: u64,
        bounds: Bounds<Pixels>,
        scale: f32,
        opacity: f32,
        mask: &Path<Pixels>,
        fallback: impl Fn() -> Vec<Bounds<Pixels>> + Send + Sync + 'static,
        f: impl FnOnce(&mut Self) -> R,
    ) -> Option<R> {
        let effect = self.masked_paint_effect(bounds, scale, opacity, mask, fallback)?;
        self.next_frame.scene.push_paint_effect(effect);
        let result = self.with_retained_paint(key, bounds, f);
        self.next_frame.scene.pop_paint_effect();
        Some(result)
    }

    /// Composite an already captured subtree without revisiting its elements.
    pub fn paint_masked_snapshot(
        &mut self,
        snapshot: &crate::PaintSnapshot,
        bounds: Bounds<Pixels>,
        scale: f32,
        opacity: f32,
        mask: &Path<Pixels>,
        ...
        self.paint_masked_snapshot_at(snapshot, bounds, scale, opacity, mask, fallback, point(px(0.), px(0.)))
    }

    /// Composite a captured subtree in the destination's current coordinates.
    pub fn paint_masked_snapshot_at(
        &mut self,
        snapshot: &crate::PaintSnapshot,
        bounds: Bounds<Pixels>,
        scale: f32,
        ...
        let Some(effect) = self.masked_paint_effect(bounds, scale, opacity, mask, fallback) else { return false; };
        let mut effect = effect;
        effect.translation = offset.scale(self.scale_factor());
        let clip = Bounds::new(bounds.origin + offset, bounds.size).scale(self.scale_factor());
        effect.clips = std::sync::Arc::new(vec![clip]);
        if let Some(mask) = &mut effect.gpu_mask {
            std::sync::Arc::make_mut(mask).content_mask.bounds = clip;
        }
        self.next_frame.scene.push_paint_effect(effect);
        self.next_frame.scene.record_retained_content(snapshot.0.clone());
        self.next_frame.scene.pop_paint_effect();
    ...
    fn masked_paint_effect(
        ...
        mask: &Path<Pixels>,
        fallback: impl Fn() -> Vec<Bounds<Pixels>> + Send + Sync + 'static,
    ) -> Option<crate::scene::PaintEffect> {
        if !self.gpu_mask_layers_enabled() {
            return None;
        }
        ...
        let mut effect = crate::scene::PaintEffect::new(
            bounds.center().scale(factor),
            scale,
            vec![bounds.scale(factor)],
        );
        effect.opacity = opacity;
        let mut mask = mask.scale(factor);
        mask.content_mask.bounds = bounds.scale(factor);
        mask.color = crate::rgb(0xffffff).into();
        effect.gpu_mask = Some(std::sync::Arc::new(mask));
        effect.mask_clips = Some(std::sync::Arc::new(crate::scene::DeferredPaintClips::new(
            move || fallback().into_iter().map(|bounds| bounds.scale(factor)).collect(),
        )));
        Some(effect)
    }

    /// Paint a subtree at a uniform scale without changing its layout. Clip bands
    /// are non-overlapping window-space rectangles, applied after scaling.
    /// Call only during paint; input regions remain the caller's responsibility.
    pub fn with_scaled_paint_clip<R>(
        &mut self,
        origin: Point<Pixels>,
        scale: f32,
        clips: &[Bounds<Pixels>],
        ...
        self.with_scaled_alpha_paint_clip(origin, scale, 1., clips, f)
    }

    /// Apply paint opacity outside a retained subtree, together with its scale
    /// and clip. Surface/video primitives follow their existing opacity behavior.
    pub fn with_scaled_alpha_paint_clip<R>(
        &mut self,
        origin: Point<Pixels>,
        scale: f32,
        opacity: f32,
        ...
        let factor = self.scale_factor();
        let mut effect = crate::scene::PaintEffect::new(
            origin.scale(factor), scale, clips.iter().map(|b| b.scale(factor)).collect(),
        );
        effect.opacity = opacity;
        self.next_frame.scene.push_paint_effect(effect);
        let result = f(self);
    ...
    pub fn paint_path(&mut self, path: Path<Pixels>, color: impl Into<Background>) {
        self.paint_path_at(&path, Point::default(), color);
    }

    /// Paint borrowed local path geometry at an offset without cloning and
    /// translating its vertices before converting them to physical pixels.
    pub fn paint_path_at(&mut self, path: &Path<Pixels>, offset: Point<Pixels>, color: impl Into<Background>) {
        self.invalidator.debug_assert_paint();

        let scale_factor = self.scale_factor();
        let content_mask = self.content_mask();
        let opacity = self.element_opacity();
        let mut path = path.scale_with_offset(scale_factor, offset);
        path.content_mask = content_mask.scale(scale_factor);
        let color: Background = color.into();
        path.color = color.opacity(opacity);
        self.next_frame
            .scene
            .insert_primitive(path);
    }
            ...
            + 'static,
    {
        ...
        let rem_size = self.rem_size();
        let scale_factor = self.scale_factor();
        if !self.text_layout_captures.is_empty() {
            // Taffy may invoke a text measurer after its view has finished
            // requesting layout. Keep that work attached to the view's cache.
            let captures = self.text_layout_captures.clone();
            return self.layout_engine.as_mut().unwrap().request_measured_layout(
                style, rem_size, scale_factor, move |known, available, window, cx| {
                    let start = window.text_system().layout_index();
                    let result = measure(known, available, window, cx);
                    let end = window.text_system().layout_index();
                    if start != end {
                        for capture in &captures {
                            let mut ranges = capture.borrow_mut();
                            // Nested measurements can finish before their caller.
                            // Merge overlapping/adjacent work, preserving gaps
                            // belonging to other views.
                            while ranges.last().is_some_and(|range| range.start >= start) {
                                ranges.pop();
                            }
    ...
    pub fn dispatch_event(&mut self, event: PlatformInput, cx: &mut App) -> DispatchEventResult {
        ...
        if self.last_input_modality != old_modality {
            // Input routing uses the current complete tree. Apply presentation
            // invalidation after dispatch, so switching to keyboard input does
            // not draw an intermediate focus-ring frame before the key action.
            let observers: Vec<_> = self.presentation_dependencies.borrow().iter()
                .filter(|(_, dependency)| dependency.modality).map(|(view, _)| *view).collect();
            for view in observers { self.invalidator.invalidate_view(view, cx); }
            self.invalidator.set_dirty(true);
        }
