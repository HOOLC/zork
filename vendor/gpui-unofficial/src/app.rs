...
impl App {
    ...
    pub(crate) fn detect_accessed_entities<R>(
        ...
        // Capture this scope's reads independently. A dependency already read
        // by a parent must still be retained when only this subtree is replayed
        // in a later frame. Set difference silently lost those dependencies.
        let previous = mem::take(self.entities.accessed_entities.get_mut());
        let result = callback(self);
        let accessed = mem::replace(self.entities.accessed_entities.get_mut(), previous);
        self.entities.accessed_entities.get_mut().extend(accessed.iter().copied());
        (result, accessed)
    }
