...
// SecTrust may synchronously wait for trustd/network checks. Keep the exact
// platform verdict, but hand the Tokio scheduler core to a replacement worker
// so a certificate check cannot stop unrelated HTTP/QUIC and timer progress.
// block_in_place keeps the CF objects on their original OS thread.
fn system_trust_call<T>(verify: impl FnOnce() -> T) -> T {
    #[cfg(target_os = "macos")]
    if tokio::runtime::Handle::try_current()
        .is_ok_and(|runtime| runtime.runtime_flavor() == tokio::runtime::RuntimeFlavor::MultiThread)
    {
        return tokio::task::block_in_place(verify);
    }
    verify()
}
...
impl Verifier {
    ...
    fn verify_certificate(
        ...
        let trust_error = match system_trust_call(|| trust_evaluation.evaluate_with_error()) {
            Ok(()) => return Ok(()),
            Err(e) => e,
        };
...
mod scheduler_tests {
    ...
    #[test]
    fn a_blocked_system_verifier_does_not_stall_other_async_tasks() {
        ...
        // Release even a broken implementation, so this regression cannot hang.
        let watchdog = std::thread::spawn(move || {
            let _ = deadline.recv_timeout(Duration::from_secs(1));
            let _ = release.send(());
        });
        let verification = runtime.spawn(async move {
            system_trust_call(|| {
                entered.send(()).unwrap();
                blocked.recv().unwrap();
            });
        ...
        let _ = finished.send(());
        runtime.block_on(verification).unwrap();
        watchdog.join().unwrap();
        assert!(
            elapsed < Duration::from_millis(500),
            "async task waited on system trust: {elapsed:?}"
        );
    }

    #[test]
    fn synchronous_and_single_thread_callers_keep_the_same_result() {
        assert_eq!(
            system_trust_call(|| Err::<(), _>("rejected")),
            Err("rejected")
        );
        let runtime = tokio::runtime::Builder::new_current_thread()
            .build()
            .unwrap();
        assert_eq!(runtime.block_on(async { system_trust_call(|| 7) }), 7);
    }
}
