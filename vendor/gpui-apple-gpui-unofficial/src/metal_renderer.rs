...
use gpui::{
    ...
    PrimitiveBatch, ScaledPixels, Scene, SceneBatch, Size, point, size,
};
...
/// Prepare the first window's device and pipelines in parallel with app startup.
/// The first native renderer adopts the owned resources; layers and drawables
/// are still created on the window thread. Later windows keep normal setup.
...
pub struct MetalRenderer {
    ...
    retained_pipeline_state: metal::RenderPipelineState,
    retained: LayerCache,
    render_stats: gpui::FrameRenderStats,
    unit_vertices: metal::Buffer,
    #[allow(clippy::arc_with_non_send_sync)]
    instance_buffer_pool: Arc<Mutex<InstanceBufferPool>>,
    sprite_atlas: Arc<MetalAtlas>,
    core_video_texture_cache: Option<core_video::metal_texture_cache::CVMetalTextureCache>,
...
impl MetalRenderer {
    ...
    pub fn new(instance_buffer_pool: Arc<Mutex<InstanceBufferPool>>, transparent: bool) -> Self {
        gpui::observe_startup("gpui.metal_begin");
        let resources = startup::take();
        gpui::observe_startup("gpui.metal_resources_taken");

        let layer = metal::MetalLayer::new();
        layer.set_device(&resources.device);
        ...
        gpui::observe_startup("gpui.metal_layer_ready");
        Self::new_internal(resources, Some(layer), !transparent, instance_buffer_pool)
    }
    ...
    pub fn new_headless(instance_buffer_pool: Arc<Mutex<InstanceBufferPool>>) -> Self {
        Self::new_internal(startup::Resources::new(), None, true, instance_buffer_pool)
    }
    ...
    fn new_internal(
        ...
        gpui::observe_startup("gpui.metal_ready");
        ...
        Self {
            ...
            surfaces_pipeline_state,
            retained_pipeline_state,
            retained: LayerCache::default(),
            render_stats: Default::default(),
            unit_vertices,
            instance_buffer_pool,
            sprite_atlas,
            core_video_texture_cache: None,
    ...
    pub fn draw(&mut self, scene: &Scene) {
        let completion = gpui::take_frame_completion();
        let renderer_started = completion.as_ref().map(|_| std::time::Instant::now());
        ...
        if completion.as_ref().is_some_and(|request| request.offscreen) {
            let request = completion.unwrap();
            let encoded = self.offscreen_target(viewport_size)
                .and_then(|target| self.render_frame(scene, &target, viewport_size));
            match encoded {
                Ok(buffer) => {
                    Self::observe_completion(&buffer, request, std::time::Duration::ZERO,
                        renderer_started.unwrap().elapsed(), self.render_stats);
                    buffer.commit();
                }
                Err(error) => log::error!("native benchmark render failed: {error:#}"),
            }
            return;
        }
        if completion.is_some() {
            // Only dedicated benchmark windows arm this observer. Measure
            // renderer capacity independently of the attached display's Hz.
        ...
        let drawable_ready = renderer_started.map(|_| std::time::Instant::now());
        ...
        if let Some(completion) = completion {
            Self::observe_completion(&command_buffer, completion,
                drawable_ready.unwrap().duration_since(renderer_started.unwrap()),
                drawable_ready.unwrap().elapsed(), self.render_stats);
        }
    ...
    fn observe_completion(buffer: &metal::CommandBufferRef, request: gpui::FrameCompletionRequest,
        drawable_wait: std::time::Duration, encoding: std::time::Duration, render: gpui::FrameRenderStats) {
        let callback = Cell::new(Some(request.callback));
        let block = ConcreteBlock::new(move |buffer: &metal::CommandBufferRef| {
            if let Some(callback) = callback.take() {
                callback(gpui::FrameCompletion {
                    completed_at: std::time::Instant::now(),
                    succeeded: buffer.status() == metal::MTLCommandBufferStatus::Completed,
                    drawable_wait, encoding, render,
                });
            }
        }).copy();
        buffer.add_completed_handler(&block);
    }
    ...
    fn render_frame(
        ...
        let fallback;
        let scene = if !layers::admitted(scene, viewport_size) {
            fallback = scene.without_retained_layers();
            self.render_stats.retained_fallback = true;
            &fallback
        } else { scene };
        self.retained.begin_frame(viewport_size, scene);
        self.encode_scene(scene, &mut writer, texture, viewport_size, &command_buffer,
    ...
    pub fn render_scene(&mut self, scene: &Scene, size: Size<DevicePixels>) -> Result<()> {
        let target_texture = self.offscreen_target(size)?;
    ...
    fn offscreen_target(&mut self, size: Size<DevicePixels>) -> Result<metal::Texture> {
        if size.width.0 <= 0 || size.height.0 <= 0 {
    ...
    fn encode_scene(
        ...
        let instance_bindings = write_instances(scene, writer)?;
        let instance_bindings = &instance_bindings;
        let retained = self.prepare_retained(scene, writer, viewport_size, command_buffer)?;
        let mut command_encoder = new_command_encoder_for_texture(
            command_buffer, texture, viewport_size,
            Some(metal::MTLClearColor::new(0., 0., 0., alpha)),
        );

        for batch in scene.render_batches() {
            let batch = match batch {
                SceneBatch::RetainedLayer(index) => {
                    self.draw_retained(&retained[index], command_encoder);
                    continue;
                }
    ...
    fn draw_paths_to_intermediate(
        ...
        let target = self.path_intermediate_texture.as_ref()
            .context("missing path intermediate texture")?;
        self.draw_paths_to_texture(paths, writer, viewport_size, command_buffer, target)
    }

    fn draw_paths_to_texture(
        &self,
        paths: &[Path<ScaledPixels>],
        writer: &mut InstanceBufferWriter,
        viewport_size: Size<DevicePixels>,
        ...
        let viewport = Bounds::new(point(ScaledPixels(0.), ScaledPixels(0.)),
            size(ScaledPixels(viewport_size.width.0 as f32), ScaledPixels(viewport_size.height.0 as f32)));
    ...
    fn draw_surfaces(
        ...
        // CoreVideo interop is only needed for video pixel buffers. Ordinary
        // windows and image/text scenes do not need a video texture cache.
        let device = &self.device;
        let texture_cache = self.core_video_texture_cache.get_or_insert_with(|| {
        ...
        for (index, surface) in surfaces.iter().enumerate() {
            ...
            let y_texture = texture_cache
                ...
                .unwrap();
            let cb_cr_texture = texture_cache
                .create_texture_from_image(
                    surface.image_buffer.as_concrete_TypeRef(),
                    None,
