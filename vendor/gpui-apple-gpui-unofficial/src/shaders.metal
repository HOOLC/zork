...
// Content textures contain premultiplied pixels. The live mask is sampled in
// destination coordinates independently of content scaling and opacity.
struct RetainedParams {
    float2 viewport;
    float2 translation;
    float scale;
    float opacity;
...
vertex RetainedVertex retained_vertex(uint index [[vertex_id]],
                                      constant RetainedParams &p [[buffer(0)]]) {
    float2 corner = float2(float(index & 1u), 0.5 * float(index & 2u));
    float2 end = p.translation + p.viewport * p.scale;
    float2 low = max(float2(0.0), max(p.clip.xy, min(p.translation, end)));
    float2 high = min(p.viewport, min(p.clip.zw, max(p.translation, end)));
    float2 position = low + corner * max(high - low, float2(0.0));
    float2 uv = p.scale == 0.0 ? float2(0.0) : (position - p.translation) / (p.viewport * p.scale);
    return {float4(position.x / p.viewport.x * 2.0 - 1.0,
                   1.0 - position.y / p.viewport.y * 2.0, 0.0, 1.0), uv};
}
fragment float4 retained_fragment(RetainedVertex input [[stage_in]],
                                  constant RetainedParams &p [[buffer(0)]],
                                  texture2d<float> content [[texture(0)]],
                                  texture2d<float> mask [[texture(1)]]) {
    constexpr sampler filtering(coord::normalized, address::clamp_to_zero, filter::linear);
    float2 position = input.position.xy;
    if (position.x < p.clip.x || position.y < p.clip.y || position.x >= p.clip.z || position.y >= p.clip.w)
        return float4(0.0);
    float coverage = p.has_mask != 0u ? mask.sample(filtering, position / p.viewport).a : 1.0;
    return content.sample(filtering, input.uv) * (p.opacity * coverage);
}
