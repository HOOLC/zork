use crate::{
    BoolExt, MacDispatcher, MacDisplay, MacWindow,
...
pub(crate) struct MacPlatformState {
    ...
    keyboard: Option<crate::MacKeyboardState>,
...
impl MacPlatform {
    pub fn new(headless: bool) -> Self {
        gpui::observe_startup("gpui.platform_begin");
        let dispatcher = Arc::new(MacDispatcher::new());
        gpui::observe_startup("gpui.dispatcher_ready");
        ...
        gpui::observe_startup("gpui.text_system_ready");

        let platform = Self(Mutex::new(MacPlatformState {
            headless,
            text_system,
        ...
        gpui::observe_startup("gpui.platform_ready");
        platform
    }
...
impl Platform for MacPlatform {
    ...
    fn keyboard_layout(&self) -> Box<dyn PlatformKeyboardLayout> {
        Box::new(self.0.lock().keyboard
            .get_or_insert_with(crate::MacKeyboardState::new).layout.clone())
    }

    fn keyboard_mapper(&self) -> Rc<dyn PlatformKeyboardMapper> {
        self.0.lock().keyboard
            .get_or_insert_with(crate::MacKeyboardState::new).mapper.clone()
    }
...
extern "C" fn on_keyboard_layout_change(this: &mut Object, _: Sel, _: id) {
    let platform = unsafe { get_mac_platform(this) };
    let mut lock = platform.0.lock();
    // Both getters must describe the same input source, including immediately
    // after a switch. The next consumer performs one main-thread TIS read.
    lock.keyboard = None;
