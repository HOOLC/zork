...
When work needs an executor, inspect available Agents. To create one, call agent.create with the known business parameters in config. Use review=true when the user needs to review or edit those parameters; missing required parameters also request user input. For configuration changes, call agent.update with changes and the inspected expected_revision. Already authorized complete operations can execute directly. Do not construct a separate form or use Chat as the business operation.
