...
impl RootView {
    ...
    fn ensure_content_tab(&mut self, kind: ContentKind, cx: &mut Context<Self>) {
        ...
        let view = cx.new(|cx| {
            zork_ui::conversation_contents::List::new(
                component_kind(kind),
        ...
        cx.subscribe(
            &view,
            move |view, _, event: &zork_ui::conversation_contents::Query, cx| {
                if let Some(tab) = view.drive.tabs.get_mut(&search_key) {
                    tab.query = event.0.clone();
                    tab.source = None;
                }
    ...
    pub(super) fn content_rows(
        &self,
        entries: Arc<Vec<ContentIndex>>,
        cx: &Context<Self>,
    ) -> zork_ui::conversation_contents::Rows {
        let items = self.drive.items.clone();
        let pages = self.drive.pages.clone();
        let locale = self.locale;
        ...
        zork_ui::conversation_contents::Rows {
            count: entries.len(),
            row: Rc::new(move |row, full| {
                let index = entries[row];
                #[cfg(feature = "headless-bench")]
                {
                    let key = match index {
                        ContentIndex::File(i) => i,
                        ContentIndex::Page(i) => items.len() + i,
                    };
                    let mut visible = indices.borrow_mut();
                    visible.insert(key);
                    count.set(visible.len());
                }
                ...
                match index {
                    ContentIndex::File(i) => {
                        ...
                        let detail = if artifact.version > 1 {
                        ...
                        zork_ui::conversation_contents::Row {
                            id: format!("{prefix}-{}", artifact.artifact_id),
                            icon: "icons/file.svg",
                            ...
                            open: Rc::new(move |cx| {
                                let _ = owner.update(cx, |v, cx| {
                                    if v.selected_session == session {
                                        v.select_artifact(artifact.clone(), cx);
                                    }
                                });
                            }),
                        }
                    ...
                    ContentIndex::Page(i) => {
                        let reference = &pages.references[i];
                        let page = reference.page.clone();
                        let detail = if reference.source_session_id.is_some() {
                            locale.text("content_handed_page").into()
                        } else {
...
fn component_kind(kind: ContentKind) -> zork_ui::conversation_contents::Kind {
    match kind {
        ContentKind::Page => zork_ui::conversation_contents::Kind::Page,
        ContentKind::File => zork_ui::conversation_contents::Kind::File,
    }
}
