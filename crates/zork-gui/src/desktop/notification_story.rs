    ...
    pub fn new(state: &str, cx: &mut Context<Self>) -> Self {
        Self {
            data: NotificationData {
                ...
                permission_label: crate::i18n::Locale::ZhCn
                    .text(if state == "denied" {
...
impl Render for NotificationStory {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        zork_ui::settings::notifications(
            self.data.clone(),
            &self.focus,
            |key| crate::i18n::Locale::ZhCn.text(key).into(),
            cx,
            |view, action, cx| {
                match action {
                    ...
                    NotificationAction::RefreshPermission => {
                        view.data.permission_label = crate::i18n::Locale::ZhCn
                            .text("notification_permission_allowed")
                            .into()
                    }
                    NotificationAction::Test | NotificationAction::SystemSettings => {
                        view.data.error = None
                    }
                }
                cx.notify();
            },
