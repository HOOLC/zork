//! Native design browser and offscreen component exporter; never packaged in Zork.app.
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(automation) = &automation {
        println!(
            "{}",
            json!({"design_pc_automation":automation.address().to_string(),"token":automation.token()})
        );
    }
