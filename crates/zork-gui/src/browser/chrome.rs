...
impl BrowserPanel {
    pub(super) fn sync_chrome(&mut self) {
        ...
        self.chrome.configure(
            zork_ui::browser_chrome::Data {
                width: self.width,
                native_pages: self.native_pages.clone(),
                active_native: self.active_native.clone(),
                tabs: self
                    .tabs
                    .iter()
                    .map(|tab| zork_ui::browser_chrome::Tab {
                        id: tab.id.clone(),
                ...
                blank: self.blank.contains(&self.host),
                expanded: self.expanded,
                error: self.error.clone(),
                granted: self.grants.contains_key(&self.host),
                grant_connected: self.grant_connected,
                can_grant: self.connection.is_some(),
    ...
    pub(super) fn render_tabs(&self, cx: &mut Context<Self>) -> gpui::AnyElement {
        self.chrome.render_tabs(cx).into_any_element()
    }
    pub(super) fn render_navigation(
        ...
        self.chrome.render_navigation(window, cx).into_any_element()
    }
    pub(super) fn render_menu(
        ...
        self.chrome.render_menu(window, cx)
    }
    pub(super) fn render_empty(&self, cx: &mut Context<Self>) -> gpui::AnyElement {
        self.chrome.render_empty(cx).into_any_element()
    }
}
    ...
    fn browser_action(
        ...
        match action {
            ...
            Action::CloseNative(id) => {
                self.close_native_page(&id, cx);
                cx.emit(super::NativePageClosed(id));
            }
            ...
            Action::CloseBlank => {
                ...
                if let Some(page) = self.native_pages.last() {
                    self.select_native_page(page.id.clone(), cx);
                } else if let Some(tab) = self.tabs.last() {
                    self.select(tab.id.clone(), cx);
                } else {
                    self.toggle(cx);
                }
            }
            Action::NewTab => {
                self.show_web_page(cx);
                self.stop_viewport();
                self.worker.clear_selection(&self.host);
                ...
                window
                    .unwrap()
                    .focus(&self.address.read(cx).focus_handle(), cx);
                cx.notify();
            }
            ...
            Action::Downloads => {
                self.chrome.menu.dismiss();
                if let Ok(rx) = self.worker.submit(|browser| browser.open_downloads()) {
            ...
            Action::ToggleInspect => {
                ...
                if self.inspecting {
                    window.unwrap().focus(&self.focus, cx);
                }
                cx.notify();
            }
            Action::Handoff => {
                self.grants.remove(&self.host);
                self.inspecting = false;
                window.unwrap().focus(&self.focus, cx);
                cx.notify();
            }
