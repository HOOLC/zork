// Adapted from the Zork Synch networking patch; see licenses/synch-network.txt.
...
impl std::fmt::Debug for RelayHttpRoute {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RelayHttpRoute").finish_non_exhaustive()
    }
}
impl RelayHttpRoute {
    ...
    pub async fn shutdown(&self) {
        let task = self
            .owner
            .as_ref()
            .and_then(|owner| owner.task.lock().unwrap().take());
...
async fn start_system_connect_proxy() -> std::io::Result<RelayHttpRoute> {
    ...
    let url =
        Url::parse(&format!("http://{}", listener.local_addr()?)).map_err(std::io::Error::other)?;
    let task = tokio::spawn(async move {
        ...
        loop {
            ...
            match accepted {
                Ok((stream, _)) if sessions.len() < 128 => {
                    failures = 0;
                    sessions.spawn(async move {
                        if let Err(error) = handle_connect(stream).await {
                            tracing::debug!(%error, "relay HTTPS session ended");
                        }
                    });
                }
                Ok(_) => {}
...
async fn handle_connect(mut inbound: TcpStream) -> std::io::Result<()> {
    ...
    let header = tokio::time::timeout(Duration::from_secs(5), async {
        ...
        while !buf.ends_with(b"\r\n\r\n") {
            ...
            buf.push(byte);
            if buf.len() > 8192 {
                return Err(std::io::Error::other("proxy headers too large"));
            }
        }
    ...
    let (host, port, path) = if tunnel {
        let (host, port) = parse_connect_target(target)
            .ok_or_else(|| std::io::Error::other("invalid CONNECT target"))?;
        (host, port, None)
    } else {
        // HTTP probes use absolute-form requests. All product payloads use TLS.
        if !matches!(method, "GET" | "HEAD") {
            return Err(std::io::Error::other("unsupported proxy request"));
        }
        let url = Url::parse(target).map_err(std::io::Error::other)?;
        if url.scheme() != "http" || !url.username().is_empty() || url.password().is_some() {
            return Err(std::io::Error::other("invalid HTTP probe"));
        }
        let host = url
            .host_str()
            .ok_or_else(|| std::io::Error::other("proxy target missing host"))?
            .to_owned();
    ...
    if tunnel {
        ...
        for line in lines
            .filter(|line| !line.is_empty() && !line.to_ascii_lowercase().starts_with("proxy-"))
        {
...
mod tests {
    ...
    async fn proxy_closes_listener_and_live_tunnels_with_its_endpoint() {
        ...
        client
            .write_all(
                format!("CONNECT {} HTTP/1.1\r\n\r\n", echo.local_addr().unwrap()).as_bytes(),
            )
            .await
            .unwrap();
        ...
        assert_eq!(
            tokio::time::timeout(Duration::from_secs(2), server.read(&mut bytes))
                .await
                .unwrap()
                .unwrap(),
            0
