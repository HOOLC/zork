...
It works only in a Chat the user opened in a Zork client. External IM (Slack and others), proactive bindings and delegated Worker tasks are refused with `mesh_invite_requires_zork_chat`; tell the user to ask from a Zork Chat. Do not retry elsewhere.
...
- Run the command yourself on the target computer (for example with shell.run on a Mesh device that can reach it), or give it only to the requesting user in this Zork Chat.
- Never post it to Slack or other external channels, issues, pull requests, logs, notes, or files in synced/shared folders.
