...
impl ModalState {
    ...
    pub fn retain<T: Clone + 'static>(
        ...
        if let Some(value) = value {
            slot.payload = Some(Box::new(value));
        }
        if !slot.open && !slot.dialog.alive() {
            slot.payload = None;
        }
        ...
        slot.payload.as_ref().map(|payload| {
            payload
                .downcast_ref::<T>()
                .expect("one presentation type per dialog ID")
                .clone()
        })
    }
    ...
    pub fn alive(&self, id: &str) -> bool {
        self.slots
            .borrow()
            .get(id)
            .is_some_and(|slot| slot.dialog.alive())
    }
    pub fn inspect(&self, id: &str) -> serde_json::Value {
        self.slots
            .borrow()
            .get(id)
            .map_or(serde_json::Value::Null, |slot| slot.dialog.inspect())
    }
    pub(super) fn render<V: 'static>(
        ...
        let (title_editor, title_action) = title_action
            .map(|action| (action.editor, Some(action.action)))
            .unwrap_or_default();
        slot.dialog
            .render_with_options(
                id,
