...
The execution workspace is the current directory for shell commands and relative file paths, and the shell environment names it as `SESSION_WORKSPACE`. Read `REPOS_ROOT` and `SHARED_FILES_ROOT` from the current shell environment for the actual repository and user shared directories; do not infer paths from another device or an old transcript.
...
- Chat attachments use `chat.post_file`; it freezes the selected files. Any readable file on this node can be sent, so a file outside the workspace does not need copying in first. Delivering a file does not require another copy into the shared directory.
