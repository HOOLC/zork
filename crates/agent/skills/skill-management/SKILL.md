...
Remote Skills use ordinary synch:// file references. Keep origin and snapshot when reading relative resources; remove only a manifest-specific root. Use file.materialize only when a program needs local paths. It returns an immutable local snapshot; copy it before editing.
