...
- 能通过 `zork-design-pc` 展示的改动，完成开发后先把本任务的交付分支 rebase 到最新 `origin/main`，解决冲突并验证最终修订；再从该修订构建 Design 客户端，更新到用户当前可直接看到的设备，启动对应示例确认。只重写自己持有的分支；设备和安装入口按本地环境配置。报告实际设备、修订和可见结果，不能以本机构建或截图代替设备更新。
