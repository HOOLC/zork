...
- 液态专项脚本操作前先核对被测控件仍使用来源材料；其形变、换层、缓存和融合断言不适用于 `modal::PlainDialog`。产品与展台窗口弹窗用 `test_playground.py` 和原生帧门禁核对正文与背景淡入、输入节点、焦点和完整开合；快照以实际已提交显示的帧为准。静态截图和代码调用关系不能代替动效验收。
...
- 按 [验证规则](../zork-validation/SKILL.md) 执行受影响的十万条混合消息/附件及渲染门禁。液态路径的原生验收核对真实缓存复用、掩码更新及回退、换层首帧像素和中途反向裁切；静态截图不能代替路径证据。PlainDialog 按其实际淡入与常规绘制路径验收。液态 Web 固定 release 产物、实际后端、视口与 DPR，独立测 CPU/GPU。
