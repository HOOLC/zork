...
- 邀请由 Station 管理，重复 join 保留身份、Profile、任务和目录；邀请/管理凭据不进报告。成员管理节点离线不等于已有节点不能执行。relay/discovery 与账号控制面不同，客户端配置覆盖不自动改写运行中的 Station。账号问题按 [中继控制面指南](../../../deploy/cloudflare/README.md) 核对运行数据目录、签发 origin、服务端撤销及已有连接；不能用本地文件删除或一次 WebSocket 升级证明完整退出与续期。切换部署还须核对实际中继进程和旧连接，Worker 发布成功不代表容器已替换。
