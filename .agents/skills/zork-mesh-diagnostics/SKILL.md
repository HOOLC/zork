...
- 启动等待分开核对本地运行时、历史恢复、本地发布与对端推送。正常关闭后重启、异常退出和数据库回滚分别验证；优化正常恢复不能跳过旧数据库的自身签名历史校验，也不能只检查传输就绪而漏掉后续源扫描入队。
- 取消异步等待不会取消阻塞池中的扫描或写入；关闭须等待这些任务和最终本地发布完成，才能生成正常关闭凭据。
- ready 后接口仍停顿时采样业务与 Mesh 工作线程，检查后台发现、TLS 平台校验等同步调用是否占满执行器；不靠增加线程数、关闭探测或放宽证书校验掩盖阻塞。
- 邀请由 Station 管理，重复 join 保留身份、Profile、任务和目录；邀请/管理凭据不进报告。成员管理节点离线不等于已有节点不能执行。relay/discovery 与账号控制面不同，客户端配置覆盖不自动改写运行中的 Station。云端只协调，不能授予 Mesh 成员身份；原生 iroh 对端身份和本地成员关系是业务访问依据。公网 relay、邀请解析和重试独立于可选账号，账号退出或封禁不能中断 Mesh。按 [中继控制面指南](../../../deploy/cloudflare/README.md) 分别验证账号会话撤销与匿名原生中继业务，不能用 WebSocket 升级证明业务可用或成员授权。切换部署核对实际中继进程和连接，Worker 发布成功不代表容器已替换。
