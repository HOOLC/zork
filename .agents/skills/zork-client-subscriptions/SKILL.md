...
- 唤醒是提示；确认后重查新版本再挂起。`changed()` 返回时已确认该批次，按它带回的 topic/domain 处理；之后再 `snapshot()` 只剩空范围。等待可取消且空闲不轮询，持锁时不调用平台或编码。日志按条数与字节有界，慢消费者不阻塞快消费者。
...
- 执行历史由节点持有，core 按需在 RAM 归并；累计统计来自 snapshot 与后续提交，不拉全档案或建客户端历史库；History 阅读页的已加载调用统计须明确范围，不冒充累计概览。首连/重连先 snapshot，详情分页不改变概览。普通 Chat 观察不启动 History loader；可见 Chat 的成员正在工作时，可共享 core History 源读取有界预览，停止或离开即释放观察。
