...
const providerLogo=id=>`<img class="provider-logo" src="../assets/providers/${providerLogoNames[id]}.svg" alt="">`;
...
document.addEventListener('submit',e=>{
 ...
 if(!Number.isInteger(context)||!Number.isInteger(output)||output<=0||context<=output){fail('输出 token 上限必须大于 0，且必须小于上下文上限');return;}
