...
早期 `archive/prototype/gui.html` 作为固定历史输入保存。当前组件和页面状态通过原生 `zork-design-pc` 展示；历史原型使用示例数据，不代表真实授权、消息、安装或升级请求。
...
`assets/manifest.json` 记录素材状态、来源和摘要。原生设计应用从该清单展示当前 SVG；Android 所需素材由 `scripts/android/export_design_assets.py` 从同一目录导出。历史 archive 不覆盖。
