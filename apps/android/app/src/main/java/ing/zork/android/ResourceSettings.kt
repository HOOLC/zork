        ...
        LazyColumn(Modifier.fillMaxSize(), contentPadding = PaddingValues(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            item {
                ...
                devices.forEach { device ->
                    device.text("error").takeIf { it.isNotBlank() }?.let { Text("${device.text("name")} · $it", color = ZorkColors.Danger) }
                    device.optJSONArray("issues").objects().forEach { Text("${device.text("name")} · ${it.text("error")}", color = ZorkColors.Danger) }
                }
            ...
            when {
                ...
                selection.query == null -> {
                    ...
                    items(rows, key = { (device, row) -> "${device.text("id")}:${row.text("id")}" }) { (device, row) ->
                        SettingsListGroup {
                            SettingsListRow(row.text("name"), subtext = "${device.text("name")} · ${resourceStatus(row.text("status"))}", detail = row.text("description"), action = {
