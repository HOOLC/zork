...
internal fun ClearDataSettings(actions: SettingsActions) {
    ...
    ZorkDialog(open, "清空客户端数据？", { if (!busy) open = false }) {
        ...
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
            ZorkButton("取消", Modifier.weight(1f), enabled = !busy, onClick = { open = false })
            ZorkButton(if (busy) "正在清空…" else "确认清空", Modifier.weight(1f), primary = true, enabled = !busy, onClick = actions.clearData)
        }
    }
}
