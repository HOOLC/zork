...
界面通过 JNI 调用 Rust 客户端核心；iroh 直接嵌入应用进程，不依赖
...
在手机的「设置 → 安卓调试」开始设置。页面按实际探测结果只显示当前需要处理的步骤，命令和端口编辑在 USB 激活说明中。手机就绪后，每台已接入的 Station 都可以同时连接，各台的授权、重试和连接状态分别显示；开启期间由前台服务保持后台连接。Station 需要 Android SDK Platform-Tools；Agent 通过工具帮助查询该 Station 上的手机和调试状态。
...
底部两个等宽入口，导航和会话各占一屏。文件卡片、片段评论与自动增高输入框使用 Android 原生控件和共享素材。
转换脚本为 `scripts/android/export_design_assets.py`，没有重绘品牌资产。

## 当前能力

- 登录与电脑相同的 Google 账号后，由 Rust core 自动发现并连接设备。
...
- 点击顶部成员名称或消息列表末尾的成员活动胶囊查看该成员的 Session 执行历史；支持常规操作分组、时间轴缩放与选区、对象跳转、翻页、重试、用量／额度概览及完整记录复制、原始 JSON 详情。执行历史只由 Rust core 按需读入内存，与聊天消息的持久缓存分开。
...
- 大模型支持订阅授权、API Key 连接、模型获取及手动编辑。凭据只发送到所属设备，不写入手机状态快照。
...
- 设置提供设备服务的只读详情；手机通过账号连接，详见 [设置范围](../../docs/design/interface.md#mobile)。
...
手机与电脑使用同一 Google 账号接入；其它执行设备使用一次性安装链接。
...
adb -s SERIAL shell am broadcast -a androidx.profileinstaller.action.INSTALL_PROFILE -n ing.zork.android.test/androidx.profileinstaller.ProfileInstallReceiver
adb -s SERIAL shell am force-stop ing.zork.android.test
adb -s SERIAL shell cmd package compile -f -m speed-profile ing.zork.android.test
adb -s SERIAL shell am start -W -n ing.zork.android.test/ing.zork.android.MainActivity
```
