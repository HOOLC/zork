...
GATES = {
    ...
    "client-frame": {
        ...
        "acceptance": "原生端共享液态目录与‘打开对话框’完整组件开合、展开中途背景淡入及实际缓存/掩码路径，页面滚动和目录内部滚动，并覆盖生产客户端会话面板及十万条混合消息滚动；滚动必须通过真实输入推动可见内容，弹窗必须到达对应的输入与运动终态。使用同一原生 Metal 渲染器与离屏目标隔离显示缓冲供应限速；输入/动画更新开始至 CPU 和 Metal GPU 均完成的全部帧严格小于预算，连续绘制完成吞吐严格超过目标 FPS，最慢连续完成间隔也须小于目标帧时长；包含首次操作与首帧，主动空闲不算掉帧；测原生渲染能力，不冒充显示器实际呈现 FPS",
        "budget_ms": 8,
        "target_fps": 120,
        "fixture": {
            "viewport": [1280, 800],
