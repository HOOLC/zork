#!/usr/bin/env python3
"""Measure both modal examples and page/directory scrolling on native Metal.
...
fixture = dict(gate['fixture'], liquid_only=True, playground=True)
...
try:
    ...
    assert raw['liquid']['viewport'] == fixture['liquid_viewport']
    cases = raw['liquid']['cases'] + raw['playground']['cases']
    assert [c['name'] for c in raw['playground']['cases']] == ['page-scroll', 'directory-scroll'] + [n for _ in range(fixture['panel_pairs']) for n in ('dialog-open', 'dialog-close')]
...
if not a.observe:
    assert report['passed'], 'Native playground performance exceeded the frame budget'
